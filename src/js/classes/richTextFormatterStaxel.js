export const StaxelRichTextFormatter = function(app) {
  const self = this;
  ...
  this.identifyTag = function(text) {
    ...
    if (text.substring(text.length - 2, text.length) === '[[')
      ...
      tag = '<<';
    else if (text.substring(text.length - 2, text.length) === '^c:')
      tag = '^c:';
  ...
  this.convert = function(text) {
    ...
    result = self._convertTag('\\[color=#(.*?)\\](.*?)\\[\\/color\\]', '^c:${match[1]};${match[2]}^c:pop;', result);
